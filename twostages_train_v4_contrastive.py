...
class HMSHBACSpecModel(nn.Module):
    ...
    def __init__(
        ...
        self.model = timm.create_model(
            model_name=model_name, pretrained=False, checkpoint_path='./pytorch_model.bin', in_chans=in_channels)
...
class ResnetGruNet(nn.Module):
    def __init__(
        ...
        # if not hybrid:
        self.fc1 = nn.Linear(in_features=linear_layer_features, out_features=1536)
...
class MulitModel(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(in_features=in_features*2, out_features=num_classes)
    ...
    def forward(self, x1,x2):
        ...
        return output, pred1, pred2, spec_feature, eeg_feature
...
def train_one_fold(CFG, fold,train_index, valid_index, output_path,stage='first'):
    ...
    contrastive_loss_func = nn.CosineEmbeddingLoss()  # Using cosine similarity for contrastive loss
...
#             batch = to_device(batch, device)
            ...
            with amp.autocast(use_amp):
                if CFG.hybrid:
                    y, y1, y2, spec_feature, eeg_feature= model(x1,x2)
                else:
                    y = model(x)
                loss = loss_func(y, t)
            ...
            embedding_2d = torch.nn.functional.normalize(spec_feature, p=2, dim=1)
            embedding_1d = torch.nn.functional.normalize(eeg_feature, p=2, dim=1)
            contrastive_target = torch.ones(embedding_1d.size(0)).to(device)  # Assuming all pairs are similar


            contrastive_loss = contrastive_loss_func(embedding_1d, embedding_2d, contrastive_target)
            loss = loss + loss1 + loss2 + contrastive_loss
