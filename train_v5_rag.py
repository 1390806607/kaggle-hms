...
class MulitModel(nn.Module):
    def __init__(
        ...
        in_features = self.spec_model.in_features
        self.fc = nn.Linear(in_features=linear_layer_features+in_features, out_features=num_classes)
    def forward(self, x1,x2):
        ...
        return output, pred1, pred2,new_features
...
def find(feature, rag_fold):
    batch = feature.shape[0]
    rags = np.load(f'/root/autodl-tmp/rag/fold{rag_fold}_rags.npy')
    rags_tensor = torch.from_numpy(rags).to(device)  
    
    cos_sim = CosineSimilarity(dim=1, eps=1e-6)
    
    ras_rags = []
    for i in range(batch):
        current_feature = feature[i].unsqueeze(0)
        
        is_self_in_rags = torch.all(rags_tensor == current_feature, dim=1)
        if torch.any(is_self_in_rags):
            rags_tensor = rags_tensor[~is_self_in_rags]

        similarity_scores = cos_sim(current_feature, rags_tensor)
        max_sim_index = torch.argmax(similarity_scores)  
        most_similar_rag = rags_tensor[max_sim_index]  
        ras_rags.append(most_similar_rag)
        
    return torch.stack(ras_rags, dim=0).squeeze()


class RAG_MulitModel(nn.Module):
    def __init__(
        self,
        model_name: str,
        pretrained: bool,
        ...
        super(RAG_MulitModel, self).__init__()
        self.spec_model = HMSHBACSpecModel(
        model_name=model_name, pretrained=pretrained, num_classes=num_classes, in_channels=spec_in_channels, hybrid=hybrid)
        self.eeg_model =  ResnetGruNet(
            kernels=kernels,
        ...
        in_features = self.spec_model.in_features
        self.rag_fc = nn.Linear(in_features=(linear_layer_features+in_features) * 2, out_features=num_classes)
        self.fold = fold if fold is not None else -1
        self.embed = nn.Parameter(
            torch.ones(linear_layer_features+in_features)
        )
    def forward(self, x1,x2):
        spec_feature,pred1 = self.spec_model(x1)
        eeg_feature, pred2 = self.eeg_model(x2)
        ...
        if self.fold > -1:
            ...
            embed = self.embed.expand(batch_size, -1)
            rag = find(new_features, self.fold)
            output = self.rag_fc(torch.cat([new_features,rag*embed],dim=1))
        else:
            output = self.fc(new_features)
        return output, pred1, pred2,new_features
...
def train_one_fold(CFG, fold,train_index, valid_index, output_path,stage='first'):
    ...
    if stage !='first':
        ...
        valid_ds = EEGDataset(valid_data, mode='valid', specs=spectrograms, eeg_specs=all_eegs, raw_eegs=all_raw_eegs, data_type=CFG.data_type,bandpass_filter=CFG.bandpass_filter)
        val_loader = DataLoader(valid_ds, shuffle=False, batch_size=CFG.batch_size*2, num_workers=10)
        model_path = './rag/'+ f"best_model_fold{fold}.pth"
        model.load_state_dict(torch.load(model_path, map_location=device), strict=False)
        print('init success')
...
#     model.load_state_dict(torch.load(model_path, map_location=device))
#     model.eval()
#     RAG_ds = EEGDataset(train, mode='valid', specs=spectrograms, eeg_specs=all_eegs, raw_eegs=all_raw_eegs, data_type=CFG.data_type,bandpass_filter=CFG.bandpass_filter)
#     RAG_dataloader = DataLoader(RAG_ds, shuffle=False, batch_size=CFG.batch_size*2, num_workers=10)

#     with torch.no_grad():
#         saved_rags = []  
#         for batch in tqdm(RAG_dataloader):
#             if CFG.hybrid:
#                 (x1, x2), t = batch
#                 x1 = x1.to(device)
#                 x2 = x2.to(device)
#                 t = t.to(device)
#                 y, y1, y2, rag = model(x1, x2)
#                 y = (y + y1 + y2)/3
#                 y_max_values, y_max_indices = torch.max(y, dim=1)
#                 t_max_values, t_max_indices = torch.max(t, dim=1)

#                 mask = (y_max_indices == t_max_indices) & (torch.abs(y_max_values - t_max_values) < 0.1)

#                 saved_rags.extend(rag[mask].cpu().numpy())
    

#     saved_rags_array = np.array(saved_rags)
#     np.save(f'fold{i}_rags.npy', saved_rags_array)