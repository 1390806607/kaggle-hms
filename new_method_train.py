"""
低质量 训练数据集是  0<vote< 10 五折，cv 是 对应的验证集，  高质量 10<vote<28  五折， cv是对应的验证集
正则项 在训练集里面加，验证集里面不加
...
https://www.kaggle.com/code/rafaelzimmermann1/no-ensemble-new-spectrograms-label-refine

step 1:Initial Training: Firstly, I trained the model exclusively with data that had high-quality labels.
"""
import math
import os
import sys
...
class CFG:
    model_name ='mixnet_s'#'tf_efficientnet_b0.ns_jft_in1k'#'tf_efficientnet_b2.ns_jft_in1k' # #'tf_efficientnet_b2'#'mixnet_xl'#'tf_efficientnet_b0'#'mixnet_s'# #'#"resnet34d"
    img_size = 512
    max_epoch = 10
    batch_size = 32
    lr = 3e-03
    ...
    data_type = 'KEGR' # K|E|R|KE|KR|ER|KER
    USE_PROCESSED = False
    hybrid= True
    # raw eeg
...
if CFG.data_type in ['R','KR','ER','KER','KEGR']: 
...
class EEGDataset(Dataset):
    ...
    def _generate_data(self, index):
        if self.data_type == 'KE':
            ...
            X2,y = self.generate_raw(index)
            X = (X1,X2)
        elif self.data_type in ['KEGR']:
            X1,y = self.generate_new_all_specs(index)
            X2,y = self.generate_raw(index)
            X = (X1,X2)
        return X,y
...
def train_one_fold(CFG, fold,train_loader, val_loader, output_path,stage='first'):
    """Main"""
    ...
    elif CFG.data_type in ['KR','ER','KER','KEGR']:
        print('-----------muLITMODEL----------------')
        model = MulitModel(
...
#             batch = to_device(batch, device)
    ...
    return fold, best_epoch, best_val_loss,model
...
if CFG.stage_type == 'kl':
    ...
    train_data = train[train['total_evaluators']>=10].reset_index(drop=True)
    train_data_2 = train[train['total_evaluators']<10].reset_index(drop=True)
    
    
gkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=CFG.seed)
splits1 = list(gkf.split(train_data,train_data[["target"]], train_data["patient_id"]))
splits2 = list(gkf.split(train_data_2, train_data_2[["target"]],train_data_2["patient_id"]))
score_list1 = []
score_list2 = []
score_list3 = []
...
for i, ((train_index, valid_index), (train_index2, valid_index2)) in enumerate(zip(splits1, splits2)):
    ...
    train_gen_ds = EEGDataset(data=train_data_.iloc[train_index], augment=False, mode='train', specs=spectrograms, eeg_specs=all_eegs, raw_eegs=all_raw_eegs, data_type=CFG.data_type, bandpass_filter=CFG.bandpass_filter,rand_filter=CFG.rand_filter)
    train_gen = DataLoader(train_gen_ds, shuffle=True, batch_size=CFG.batch_size, num_workers=10,drop_last=False)

    valid_gen_ds = EEGDataset(train_data_.iloc[valid_index], mode='valid', specs=spectrograms, eeg_specs=all_eegs, raw_eegs=all_raw_eegs, data_type=CFG.data_type,bandpass_filter=CFG.bandpass_filter)
    valid_gen = DataLoader(valid_gen_ds, shuffle=False, batch_size=CFG.batch_size*2, num_workers=10)


    print(f'### Train size: {len(train_index)}, Valid size: {len(valid_index)}')
    print('#'*25)
    fold, best_epoch, best_val_loss, model = train_one_fold(CFG, i,train_gen, valid_gen, output_path)
    score_list1.append((fold, best_epoch, best_val_loss))
    
    exp_dir_path = Path(f"fold{fold}")
    best_model_path = exp_dir_path / f"snapshot_epoch_{best_epoch}.pth"
    copy_to = f"./best_model_fold{fold}.pth"
    ...
    train_2_index_total_gen_ds = EEGDataset(data=train_data_2_.iloc[train_index2], augment=False, mode='train', specs=spectrograms, eeg_specs=all_eegs, raw_eegs=all_raw_eegs, data_type=CFG.data_type, bandpass_filter=CFG.bandpass_filter,rand_filter=CFG.rand_filter)
    train_2_index_total_gen = DataLoader(train_2_index_total_gen_ds, shuffle=False, batch_size=CFG.batch_size, num_workers=10,drop_last=False)
    

    pseudo_labels_2 = run_inference_loop(model, train_2_index_total_gen, device)
    ...
    train_3_index_total_gen_ds = EEGDataset(data=train_data_3_.iloc[train_index], augment=False, mode='train', specs=spectrograms, eeg_specs=all_eegs, raw_eegs=all_raw_eegs, data_type=CFG.data_type, bandpass_filter=CFG.bandpass_filter,rand_filter=CFG.rand_filter)
    
    train_3_index_total_gen = DataLoader(train_3_index_total_gen_ds, shuffle=False, batch_size=CFG.batch_size, num_workers=10,drop_last=False)
    
    pseudo_labels_3 = run_inference_loop(model, train_3_index_total_gen, device)
    ...
    train_gen_2_ds = EEGDataset(
        data=pd.concat([train_data_3_.iloc[train_index_60], train_data_2_.iloc[train_index2]]), 
        augment=False, 
        mode='train', 
        specs=spectrograms, 
        eeg_specs=all_eegs, 
        raw_eegs=all_raw_eegs, 
        data_type=CFG.data_type, 
        bandpass_filter=CFG.bandpass_filter,
        rand_filter=CFG.rand_filter
    )
    
    train_gen_2 = DataLoader(train_gen_2_ds, shuffle=True, batch_size=CFG.batch_size, num_workers=10,drop_last=False)
    
    

    valid_gen_2_ds = EEGDataset(
    ...
    valid_gen_2 = DataLoader(valid_gen_2_ds, shuffle=False, batch_size=CFG.batch_size*2, num_workers=10)
    


    # Rebuild the high quality data generator with 50% of the labels refined
    train_gen_ds = EEGDataset(
        data=train_data_.iloc[train_index], 
        augment=False, 
        mode='train', 
        specs=spectrograms, 
        eeg_specs=all_eegs, 
    ...
    fold, best_epoch, best_val_loss, model = train_one_fold(CFG, i,train_gen_2, valid_gen_2, output_path)
    score_list2.append((fold, best_epoch, best_val_loss))
    exp_dir_path = Path(f"fold{fold}")
    best_model_path = exp_dir_path / f"snapshot_epoch_{best_epoch}.pth"
    copy_to = f"./best_model_fold{fold}.pth"
    shutil.copy(best_model_path, copy_to)
    ...
    fold, best_epoch, best_val_loss, model = train_one_fold(CFG, i,train_gen, valid_gen, output_path,stage='sec')
    score_list3.append((fold, best_epoch, best_val_loss))
...
for (fold_id, best_epoch, _) in score_list3:
...
all_true = []
valid_loaders = []
for i, (train_index, valid_index) in enumerate(splits1): 
    print('#'*25)
    print(f'### Fold {i+1}')
