...
class HMSHBACSpecModel(nn.Module):
    ...
    def __init__(
        ...
        self.model = timm.create_model(
            model_name=model_name, pretrained=False, checkpoint_path='./mixnets.bin', in_chans=in_channels)
...
class HopefullNet(nn.Module):
    ...
    def __init__(self, num_classes : int = 5,
        ...
        self.Layer1 = nn.Sequential(
            ...
            nn.Conv1d(in_channels= in_channels, out_channels= 32, kernel_size= self.kernel_size_0, stride = 1, padding= 0),
        ...
        self.FC1 =nn.Sequential(
            nn.Linear(31456, linear_layer_features),
            nn.LeakyReLU(inplace = True),
            nn.Dropout(self.drop_rate),            
        )
        self.FC2 = nn.Sequential(
            nn.Linear(linear_layer_features, 148),
            nn.LeakyReLU(inplace = True),
            nn.Dropout(self.drop_rate),
            nn.Linear(148, 74),
            nn.LeakyReLU(inplace = True),
...
class MulitModel(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(in_features=in_features+linear_layer_features, out_features=num_classes)
